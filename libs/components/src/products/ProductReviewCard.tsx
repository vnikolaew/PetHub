...
const ProductReviewCard: FC<ProductReviewCardProps> = ({ review }) => {
   return (
      <div
         className={`flex items-start p-6 rounded-xl border border-gray-200 shadow-md gap-4`}
      >
         <Image
         ...
         >
            ...
            <p className={`text-[1.1rem] text-gray-500`}>{review.reviewText}</p>
         </div>
         <div>
            <div className={`flex items-center gap-1`}>
               ...
               {Array.from({ length: 5 - review.rating }).map((_, i) => (
                  <StarIcon height={20} width={20} color={"orange"} key={i} />
               ))}
            </div>
         </div>
      </div>
